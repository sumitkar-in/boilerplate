...
export class KnowledgeBotService {
  ...
  private writeSse(res: Response, chunk: AiChatStreamChunk): void {
    res.write(formatSseJsonFrame(chunk)); // lgtm[js/reflected-xss] SSE frames are served as text/event-stream with nosniff and JSON HTML-sensitive chars escaped.
  }
